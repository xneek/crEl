			...
			for(k in args[i]){
			  if(k==='e' || k==='events'){
				...
				if( typeof args[i][k] === 'function' ){
					e[k] = (function(func){ return function(){ func.apply(e, arguments); } })(args[i][k])
				} else {
					e[k] = args[i][k];
				}
			  } else if(k==='d' || k==='data'){
				for(x in args[i][k]){
					if('dataset' in e){
						...
						e.setAttribute('data-'+ (x.replace(/([A-Z])/g, function(string) { return '-' + string.toLowerCase();})),args[i][k][x]);
					}
				}
			  } else  if(k === 'c' || k === 'class'){
				  if('classList' in e){
